...
#  ptreelist = [file for file in list_of_ptrees if file.endswith('.ptre')]
...
# to run likelihood trees using starting tree:
    popen = subprocess.Popen(['raxmlHPC-PTHREADS', '-T', NUMTHREADS, '-f', 'e','-s', file_name, '-m', newEvo, '-n', gene_file[:-4], '-t', ptre])   
...
options (listed below) default to 'None' if omitted
    ...
    runRaxml(list_of_species_files, unescape(options.evo), evoDict, list_of_ptrees)
